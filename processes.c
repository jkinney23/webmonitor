...
/* Return the resident set size (RSS), in kilobytes, or process PID.
 * Return -1 on failure.  */
...
static int get_rss (pid_t pid)
{
	...
	/* Generate the name of the process's "statm" entry in it /proc
	 * directory.  */
	snprintf (file_name, sizeof (file_name), "/proc/%d/statm", (int) pid);
	/* Open it.  */
	fd = open (file_name, O_RDONLY);
	if (fd == -1)
		/* Couldn't open it; perhaps this process no longer exsts.  */
		return -1;
...
static char* page_end = 
	...
	"<a href=\"/issue\" class=\"btn btn-default btn-block\" role=\"button\">Previous</a>"
	"</div>"
	"<div class=\"col-md-2\">"
	"<a href=\"/\" class=\"btn btn-default btn-block\" role=\"button\">Home</a>"
	"</div>"
	"<div class=\"col-md-2\">"
	"<a href=\"/time\" class=\"btn btn-default btn-block\" role=\"button\">Next</a>"
	"</div>"
	" </body>\n"
	"</html>\n";
...
void module_generate (int fd)
{
	...
	/* Set up the iovec array.  We'll fill this with buffers that'll be
	...
	while (1) {
		...
		if (process_info == NULL)
			...
			process_info = "<tr><td colspan=\"5\">ERROR<\td><\tr>";
