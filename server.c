...
/* Http response, header, and body templat, indicating that the
...
/* HTTP response, header, boy template, indicating that the
...
/* Handler for SIGCHILD, to clean up child processes that have
 * terminated.  */
...
static void handle_get (int connection_fd, const char* page) // page is called by "url" in handle_connection
{
	...
	if (module == NULL) {
		...
		/* The requested module was loaded successfuly.  */
		
		/* Send the HTTP response indicating success, and the HTTP header
		 * for an HTML page.  */
		write (connection_fd, ok_response, strlen (ok_response));
		/* Invoke the module, which will generate HTML ouput and send it
...
static void handle_connection (int connection_fd)
{
	...
	if (bytes_read > 0) {
		...
		/* Check the protocol field.  We understand version 1.0 and
		 * 1.1.  */
...
void server_run (struct in_addr local_address, uint16_t port)
{
	...
	/* Install a handler for SIGCHILD that cleans up child processes that
	 * have terminated.  */
...
//---------------update to here----------------------------------
	...
	if (verbose) {
		...
		printf("server listening on %s:%d\n",
			   ip4, (int) ntohs (socket_address.sin_port));
	}
