...
struct server_module* module_open (const char* module_name)
{
	...
	free (module_path);
	if (handle == NULL) {
		/* Failed; either this path doesn exist or it isn't a shared
		 * library.  */
		return NULL;
	}
	...
	if (module_generate == NULL) {
		/* The symbol is missing. While this is a shared library, ir
		 * probably isn't a servermodule.  Close up and indicat failure.  */
		dlclose (handle);
		return NULL;
	}
